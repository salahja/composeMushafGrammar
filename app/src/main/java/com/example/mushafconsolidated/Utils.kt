...
class Utils {
   ...
   database = context?.let { QuranAppDatabase.getInstance(it) }!!
    ...
    fun getQuranNounsbyroot(id: String, ): List<NounCorpus> {
        Log.d( TAG, "getQuranNouns: started")
        return database.NounCorpusDao().NounbyRoot(id)
    }
    ...
    fun getRootVerbDetails(tid: String): List<RootVerbDetails>? {
        val sqlverb: String =
            ("SELECT CorpusExpand.araone ||CorpusExpand. aratwo ||CorpusExpand. arathree || CorpusExpand.arafour ||CorpusExpand.arafive as arabic,\n" +
                    ...
                    "\tand qurandictionary.wordno = CorpusExpand.wordno  AND qurandictionary.surah=wbw.surah and qurandictionary.ayah=wbw.ayah\n" +
                    "and qurandictionary.wordno=wbw.wordno and qurandictionary.surah=chaptersana.chapterid and qurans.surah=qurandictionary.surah and qurans.ayah=qurandictionary.ayah and qurandictionary.rootarabic=  \""
                    + tid + "\"")
        val query: SimpleSQLiteQuery = SimpleSQLiteQuery(sqlverb)
        //  List<Book> result = booksDao.getBooks(query);
    ...
    fun getwordroot(cid :Int,aid: Int,id: String): List<CorpusEntity> {
        return database.getCorpusExpandDao().getwordroot(cid,aid,id)
    }
...
*/
    ...
    fun getQuranNounByfirstletter(id: String): List<NounCorpus> {
        return  database.NounCorpusDao().getQuranNounByfirstletter(id)
    }
